...
public class RentACatUnitTest {
	...
	public void setUp() throws Exception {
		...
		PrintStream ps = new PrintStream(out); // this printstream writes to 'out'
		System.setOut(ps); // this redirects terminal output to 'ps'
	}
	...
	public void testGetCatNullNumCats0() throws Exception {
		out.reset();
		Class<?> rentACatClass = r.getClass(); // class object of r

		//get private method getCat(ind id)
		Method getCatMethod = rentACatClass.getDeclaredMethod("getCat", int.class); 

		getCatMethod.setAccessible(true); // make method accessible
	...
	public void testReturnCatNumCats3() {
		...
		// set up behavior to change rented status when returnCat() is called
		doAnswer(invocation -> {
			when(c2.getRented()).thenReturn(false);
			return null;
		}).when(c2).returnCat();
